...
## Outputs
A comment will be placed (or updated) in the pull request. The comment will be the supplied `body` (or contents of `body_path`) as well as a hidden identifier that is placed at the beginning of the body (`check-name`).
...
The check supplied here should match the name provided in the `check-name` input field on the action. If the action has been triggered before, it **should** show up in the "Suggestions" tab as you type it. If not, then the name can be supplied and it **should** detect on the first run of the action
