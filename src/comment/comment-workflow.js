...
async function commentWorkflow(token) {
	...
	const hideOnEmpty =
		(core.getInput("hide-on-empty", { required: false }) || "true") === "true";
	...
	try {
		...
		if (!comment) {
			if (conclusion === "skipped") {
				...
				core.getInput("sync-conclusion", { required: false }) === "true" &&
				conclusion === "success"
			) {
				logger.debug(
			...
			const updateMode =
				core.getInput("update-mode", { required: false }) || "create";
			logger.debug(
			...
			if (updateMode === "create") {
				...
				// If the body went from empty to populated
				if (
					comment.body.replace(/^[\s\S]*-->\s*/, "").trim() === "" &&
					commentToUpdateWith !== "" &&
					hideOnEmpty
				) {
					logger.debug(
				...
				// If the body went from anything to empty, hide the comment
				...
				// If sync-conclusion is enabled, hide/unhide the comment based on the conclusion status
				else if (
					core.getInput("sync-conclusion", { required: false }) === "true"
				) {
