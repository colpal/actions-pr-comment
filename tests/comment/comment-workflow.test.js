...
const {
	hideComment,
	unhideComment,
} = require("../../src/comment/comment-visibility.js");
...
describe("comment-workflow", () => {
	...
	it("should log info when no existing comment is found", async () => {
		...
		expect(logger.debug).toHaveBeenCalledWith(
			"No existing comment found, posting a new comment.",
		);
	});
	...
	it("should log info when existing comment is found", async () => {
		...
		expect(logger.debug).toHaveBeenCalledWith(
			expect.stringContaining("Comment found. ID: 1. Update Mode: replace"),
		);
	});
	...
	it("should handle error thrown by findComment", async () => {
		...
		expect(logger.error).toHaveBeenCalledWith(
			expect.stringContaining(
				"Error occurred during comment workflow: findComment error",
			),
		);
	});
	...
	it("should handle error thrown by postComment", async () => {
		...
		expect(logger.error).toHaveBeenCalledWith(
			expect.stringContaining(
				"Error occurred during comment workflow: postComment error",
			),
		);
	});
	...
	it("should handle error thrown by updateComment", async () => {
		...
		expect(logger.error).toHaveBeenCalledWith(
			expect.stringContaining(
				"Error occurred during comment workflow: updateComment error",
			),
		);
	});
	...
	it("should handle error thrown by hideComment", async () => {
		...
		expect(logger.debug).toHaveBeenCalledWith(
			expect.stringContaining("Comment found. ID: 1. Update Mode: create"),
		);
		expect(hideComment).toHaveBeenCalledWith(token, mockComment, "OUTDATED");
		expect(logger.error).toHaveBeenCalledWith(
			expect.stringContaining(
				"Error occurred during comment workflow: hideComment error",
			),
		);
	...
	it('should call hideComment and postComment when update-mode is "create"', async () => {
		...
		expect(updateComment).not.toHaveBeenCalled();
		expect(logger.debug).toHaveBeenCalledWith(
			"Update mode is 'none', skipping comment update.",
		);
	});
	...
	it('should call nothing when conclusion is "cancelled"', async () => {
		...
		expect(logger.debug).toHaveBeenCalledWith(
			"Conclusion is 'cancelled', skipping comment workflow.",
		);
	});
	...
	it('should exit early and not hide previous comment when conclusion is "skipped" and sync-conclusion is false', async () => {
		...
		expect(findComment).toHaveBeenCalled();
		expect(logger.debug).toHaveBeenCalledWith(
			"Conclusion is 'skipped', skipping comment update.",
		);
	});
	...
	it('should unhide comment if update mode is not "create" and body is populated', async () => {
		const mockComment = { id: 1, body: "" };
		findComment.mockResolvedValue(mockComment);
		hideComment.mockResolvedValue();
		postComment.mockResolvedValue();
		updateComment.mockResolvedValue();
	...
	it('should not unhide comment if update mode is not "create" and body is populated but hide-on-empty is false (because it should not have been hidden in first place', async () => {
		const mockComment = { id: 1, body: "" };
		findComment.mockResolvedValue(mockComment);
		hideComment.mockResolvedValue();
		postComment.mockResolvedValue();
