...
const { getCommentBody } = require("../../src/util/util.js");
...
describe("postComment", () => {
  ...
  it("skips posting if not a pull request", async () => {
    ...
    await expect(
      postComment(octokit, "owner", "repo", "identifier"),
  ...
  it("handles errors thrown by getCommentBody", async () => {
    getCommentBody.mockImplementationOnce(() => {
  ...
  it("should not create a comment if commentBody is empty and hideOnEmpty is true", async () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith(
      "Comment body is empty. Skipping comment post.",
    );
    expect(getCommentBody).toHaveBeenCalled();
