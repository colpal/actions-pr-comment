...
const { getCommentBody, renderCommentBody } = require("../../src/util/util.js");
...
describe("getCommentBody", () => {
  ...
  it("reads comment body from file when only comment-body-path is provided", () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith(
      "Reading comment body from file: path/to/file.md",
    );
  ...
  it("throws error if file cannot be read", () => {
    core.getInput.mockImplementation((key) =>
      key === "comment-body-path" ? "bad/path.md" : "",
    );
    fs.readFileSync.mockImplementation(() => {
      throw new Error("File not found");
    });
    expect(() => getCommentBody()).toThrow(
      "Could not read file at path: bad/path.md. Error: File not found",
    );
  });
  ...
  it("removes BOM from file content if present", () => {
    ...
    expect(getCommentBody()).toBe("File comment");
    expect(logger.debug).toHaveBeenCalledWith(
      "Reading comment body from file: bomfile.md",
    );
  ...
  it("returns empty string when file is empty", () => {
    ...
    expect(getCommentBody()).toBe("");
    expect(logger.debug).toHaveBeenCalledWith(
      "Reading comment body from file: emptyfile.md",
    );
    expect(logger.debug).toHaveBeenCalledWith(
      "Comment body path's file is empty.",
    );
  ...
  it("returns empty string when file content is empty after BOM removal", () => {
    ...
    expect(getCommentBody()).toBe("");
    expect(logger.debug).toHaveBeenCalledWith(
      "Reading comment body from file: bom-only.md",
    );
    expect(logger.debug).toHaveBeenCalledWith(
      "Comment body path's file is empty.",
    );
  ...
  it("throws error if file content is not a string", () => {
    core.getInput.mockImplementation((key) =>
      key === "comment-body-path" ? "notstring.md" : "",
    );
  ...
  it("handles file content with new lines correctly", () => {
    ...
    expect(logger.debug).toHaveBeenCalledWith(
      "Reading comment body from file: multiline.md",
    );
  ...
  it("returns un-rendered comment from comment-body even if render-markdown is false", () => {
    ...
    expect(renderCommentBody("Line 1\nLine 2")).toBe(
      "<pre id=render-markdown-false>Line 1\nLine 2</pre>",
    );
    expect(getCommentBody()).toBe(
      "<pre id=render-markdown-false>Line 1\nLine 2</pre>",
    );
  ...
  it("returns un-rendered comment from comment-body-path even if render-markdown is false", () => {
    ...
    const multilineContent = "Line 1\nLine 2\nLine 3";
    fs.readFileSync.mockReturnValue(multilineContent);
    expect(renderCommentBody(multilineContent)).toBe(
      "<pre id=render-markdown-false>Line 1\nLine 2\nLine 3</pre>",
    );
    expect(getCommentBody()).toBe(
      "<pre id=render-markdown-false>Line 1\nLine 2\nLine 3</pre>",
